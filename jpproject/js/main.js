...
const PROJ = [
  { id:'p1', no:'PRJ-2026-001', name:'智能客服平台建设', type:'external', product:'产品A', leader:'p1', status:'active', start:'2026-03-01', end:'2026-09-30', desc:'构建基于AI的智能客服平台，实现7×24小时自动应答，提升客户服务效率。', members:[
    {pid:'p1',roles:['项目负责人'],attr:'国智',hours:h('2026-03','2026-09',120),total:840},
    {pid:'p2',roles:['产品经理'],attr:'国智',hours:h('2026-03','2026-05',100),total:300},
    {pid:'p4',roles:['前端开发'],attr:'合作方',hours:h('2026-04','2026-07',100),total:400},
    {pid:'p8',roles:['后端开发'],attr:'国智',hours:h('2026-04','2026-08',100),total:500}
  ]},
  { id:'p2', no:'PRJ-2026-002', name:'数据中台升级改造', type:'internal', product:'产品B', leader:'p3', status:'active', start:'2026-02-15', end:'2026-08-31', desc:'对公司数据中台进行架构升级，提升数据处理能力与稳定性。', members:[
    {pid:'p3',roles:['项目负责人','架构师'],attr:'国智',hours:h('2026-02','2026-08',120),total:840},
    {pid:'p6',roles:['数据分析师'],attr:'国智',hours:h('2026-03','2026-06',100),total:400},
    {pid:'p7',roles:['产品经理'],attr:'合作方',hours:h('2026-02','2026-04',100),total:300}
  ]},
  { id:'p3', no:'PRJ-2026-003', name:'移动端App重构', type:'external', product:'产品A', leader:'p2', status:'completed', start:'2026-01-01', end:'2026-06-30', desc:'对现有移动端App全面重构，采用Flutter跨平台方案。已按时交付。', members:[
    {pid:'p2',roles:['项目负责人'],attr:'国智',hours:h('2026-01','2026-06',120),total:720},
    {pid:'p9',roles:['UI设计'],attr:'合作方',hours:h('2026-01','2026-02',100),total:200},
    {pid:'p4',roles:['移动端开发'],attr:'国智',hours:h('2026-02','2026-06',100),total:500}
  ]},
  { id:'p4', no:'PRJ-2025-018', name:'ERP系统对接项目', type:'internal', product:'产品C', leader:'p5', status:'active', start:'2025-11-01', end:'2026-07-31', desc:'完成公司ERP系统与各业务系统的接口对接与数据集成。', members:[
    {pid:'p5',roles:['项目负责人'],attr:'国智',hours:h('2025-11','2026-07',120),total:1080},
    {pid:'p10',roles:['业务分析师'],attr:'合作方',hours:h('2025-11','2026-01',100),total:300},
    {pid:'p1',roles:['技术顾问'],attr:'国智',hours:h('2026-01','2026-03',100),total:300}
  ]},
  { id:'p5', no:'PRJ-2026-004', name:'微服务架构迁移', type:'internal', product:'产品B', leader:'p8', status:'suspended', start:'2026-04-01', end:'2026-12-31', desc:'将现有单体应用逐步迁移至微服务架构，分三阶段实施。因资源冲突暂挂。', members:[
    {pid:'p8',roles:['项目负责人'],attr:'国智',hours:h('2026-04','2026-06',120),total:360},
    {pid:'p4',roles:['后端开发'],attr:'国智',hours:h('2026-04','2026-06',100),total:300}
  ]},
  { id:'p6', no:'PRJ-2025-012', name:'办公自动化系统', type:'external', product:'产品C', leader:'p6', status:'cancelled', start:'2025-08-01', end:'2026-03-31', desc:'建设公司内部办公自动化系统。因战略调整已终止。', members:[
    {pid:'p6',roles:['项目负责人'],attr:'国智',hours:h('2025-08','2026-03',120),total:960}
  ]},
  { id:'p7', no:'PRJ-2026-005', name:'AI推荐引擎开发', type:'external', product:'产品A', leader:'p1', status:'active', start:'2026-05-01', end:'2026-11-30', desc:'开发深度学习推荐引擎，应用于内容分发与商品推荐，目标提升转化率30%。', members:[
    {pid:'p1',roles:['项目负责人'],attr:'国智',hours:h('2026-05','2026-11',120),total:840},
    {pid:'p8',roles:['算法工程师'],attr:'国智',hours:h('2026-05','2026-09',100),total:500},
    {pid:'p2',roles:['产品经理'],attr:'合作方',hours:h('2026-05','2026-07',100),total:300}
  ]},
  { id:'p8', no:'PRJ-2026-006', name:'安全合规审计平台', type:'internal', product:'产品B', leader:'p7', status:'closed', start:'2026-01-15', end:'2026-06-15', desc:'建立安全合规审计平台，满足行业监管要求。已于2026年6月通过结项审批。', members:[
    {pid:'p7',roles:['项目负责人'],attr:'国智',hours:h('2026-01','2026-06',120),total:720},
    {pid:'p10',roles:['合规专员'],attr:'合作方',hours:h('2026-01','2026-06',100),total:600},
    {pid:'p3',roles:['安全专家'],attr:'国智',hours:h('2026-01','2026-03',100),total:300}
  ]},
];
  ...
  document.getElementById('content').innerHTML = '\
    ...
    <div class="tbl-wrap">\
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:10px 20px;border-bottom:1px solid var(--bd);background:var(--mu);flex-wrap:wrap;">\
        ...
        <select onchange="setFilter(\'product\',this.value)" style="padding:5px 10px;border-radius:var(--r);border:1.5px solid var(--bd);font-size:12px;font-family:var(--font);background:var(--surf);">\
        ...
        <select onchange="setFilter(\'type\',this.value)" style="padding:5px 10px;border-radius:var(--r);border:1.5px solid var(--bd);font-size:12px;font-family:var(--font);background:var(--surf);">\
        ...
        <select onchange="setFilter(\'leader\',this.value)" style="padding:5px 10px;border-radius:var(--r);border:1.5px solid var(--bd);font-size:12px;font-family:var(--font);background:var(--surf);">\
...
function startEdit() {
  ...
  document.getElementById('de').innerHTML='\
    ...
    '+(p.status==='active'?'<div class="card" style="margin-top:20px;"><div class="card-hd">项目状态变更（REQ-PM-004）</div>\
      ...
      <div class="fg"><label>变更为目标状态</label><select id="e-status" onchange="var w=document.getElementById(\'e-status-reason-wrap\');if(w)w.style.display=this.value?\'\':\'none\';"><option value="" selected>不变更</option><option value="completed">已完成 — 项目正常完成</option><option value="suspended">挂起 — 暂时中止</option><option value="cancelled">已作废 — 项目作废</option></select></div>\
...
function showMemModal() {
  ...
  var rows = '';
  tmpMems.forEach(function (mem, i) {
    const pp = PPL.find(function (x) { return x.id === mem.pid; });
    var total = 0;
    months.forEach(function (mon) { total += parseFloat(mem.hours?.[mon]) || 0; });
    rows += '<tr data-idx="'+i+'">\
      <td style="text-align:center;">'+(i+1)+'</td>\
      <td><span class="mav" style="background:'+(pp?.av||'#2563EB')+'">'+((pp?.name||'?')[0])+'</span>'+(pp?.name||'—')+'</td>\
      <td><div class="chip-grp" data-name="roles" data-multi="1" style="min-width:140px;">'+ROLES.map(function (r) { var isSel=(mem.roles||[]).includes(r); return '<span class="chip'+(isSel?' sel':'')+'" data-value="'+r+'" onclick="toggleChip(this,1)">'+r+(isSel?'<span class="ch-x">×</span>':'')+'</span>'; }).join('')+'</div></td>\
  ...
  if (!tmpMems.length) rows = '<tr><td colspan="'+(4+months.length+2)+'" style="text-align:center;color:var(--tx3);padding:40px;">暂无成员，请点击右上角添加</td></tr>';
...
function showAddMemPicker() {
  const avail = PPL.filter(function (x) { return !tmpMems.some(function (mem) { return mem.pid === x.id; }); });
...
function confirmAddMems(btn) {
  ...
  checked.forEach(function (cb) {
    var hours = {};
    var projectMonths = getMonths(editData?.start || selProj?.start, editData?.end || selProj?.end);
    projectMonths.forEach(function (mon) { hours[mon] = 0; });
    tmpMems.push({ pid: cb.dataset.pid, roles: ['成员'], attr: '国智', hours: hours, total: 0 });
  });
...
function saveMems(btn) {
  ...
  rows.forEach(function (row) {
    ...
    var roleGrp = row.querySelector('.chip-grp[data-name="roles"]');
    var roles = readChips(roleGrp, true);
    var attrGrp = row.querySelector('.chip-grp[data-name="attr"]');
    var attr = readChips(attrGrp, false) || '国智';
    var hours = {}, total = 0;
    ...
    tmpMems[idx] = { ...tmpMems[idx], roles: roles.length ? roles : ['成员'], attr: attr, hours: hours, total: total };
  });
  btn.closest('.mod').remove();
...
function renderMerge() {
  document.getElementById('content').innerHTML = '\
    ...
    <div class="tbl-wrap">'+(MERGE.length===0?'<div class="empty">暂无待合并项目</div>':'<table><thead><tr><th>类型</th><th>编号</th><th>名称</th><th>来源</th><th>同步时间</th><th>操作</th></tr></thead><tbody>'+MERGE.map(function (m) { var tagClass=m.type==='setup'?'b':m.type==='close'?'r':'gr'; var tagText=m.type==='setup'?'立项':m.type==='close'?'结项':'手动创建'; return '<tr><td><span class="tag '+tagClass+'">'+tagText+'</span></td><td>'+m.no+'</td><td><span class="ct">'+m.f.name+'</span></td><td>'+m.src+'</td><td style="font-size:12px;color:var(--tx2);">'+m.dt+'</td><td class="ca"><button class="btn btn-p btn-sm" onclick="startMerge(\''+m.id+'\')">合并项目信息</button>'+(m.type!=='close'?'<button class="btn btn-o btn-sm" onclick="quickNew(\''+m.id+'\')">新建合并项目</button>':'')+'</td></tr>'; }).join('')+'</tbody></table>')+'</div>';
  page='merge';
}
...
function doCreate(btn) {
  var nm=document.getElementById('ncn').value.trim(); if(!nm){toast('请输入项目名称','err');return;}
...
function quickNew(mid) {
  var mi=MERGE.find(function (m) { return m.id===mid; }); if(!mi) return;
...
function renderMergeStep() {
  var mi=selMerge; var ct=document.getElementById('content');
  var isClose=mi.type==='close';
  var stepHTML=(function () {
    if(isClose){
      var c=function (n) { return mergeStep===n?'on':mergeStep>n?'ok':''; };
  ...
  if(mergeStep===1) {
    var cf=PROJ.filter(function (p) { return p.no===mi.f.no||p.name.includes(mi.f.name.substring(0,4))||mi.f.name.includes(p.name.substring(0,4)); });
    var srcTxt=mi.type==='setup'?'飞书立项流程':mi.type==='close'?'飞书结项流程':'手动创建';
  ...
  if(mergeStep===2) {
    var tgt=PROJ.find(function (p) { return p.id===mi._cid; });
    var fields=[{k:'name',l:'项目名称'},{k:'no',l:'项目编号'},{k:'type',l:'项目类型'},{k:'product',l:'所属产品'},{k:'leader',l:'项目负责人'},{k:'desc',l:'项目描述'}];
...
function doMerge(){
  var mi=selMerge; var tgt=mi._cid?PROJ.find(function (p) { return p.id===mi._cid; }):null;
...
/* ═══════════════════════════════════════
   ...
   ═══════════════════════════════════════ */
function renderAppr() {
  var pending=APPR.filter(function (a) { return a.st==='pending'; }); var done=APPR.filter(function (a) { return a.st!=='pending'; });
  document.getElementById('content').innerHTML='\
...
function doAppr(aid,result){
  ...
  var a=APPR.find(function (x) { return x.id===aid; });if(!a)return;
  a.st='approved';a.result='通过';a.rv='当前用户（PMO）';a.rd=new Date().toISOString().split('T')[0];
  var p=PROJ.find(function (x) { return x.id===a.pid; });if(p){var m={'已完成':'completed','挂起':'suspended','已作废':'cancelled'};p.status=m[a.to]||p.status;}
...
function addRole() {
  var inp = document.getElementById('newRoleInput'); var v = inp.value.trim();
...
function renderUserTb(f) {
  f = f || '';
  var list=[...USER_DATA];
  if(f){var q=f.toLowerCase();list=list.filter(function (u) { var p=PPL.find(function (x) { return x.id===u.pid; }); return (p?.name||'').toLowerCase().includes(q)||u.rl.toLowerCase().includes(q); });}
  var tc={admin:'b',pmo:'g',lead:'y',user:'gr'};var perm={admin:'全部权限',pmo:'审批+待合并操作',lead:'查看+编辑项目',user:'仅查看项目'};
  document.getElementById('userTb').innerHTML=list.map(function (u) { var p=PPL.find(function (x) { return x.id===u.pid; }); return '<tr><td><input type="checkbox" data-uid="'+u.id+'" onchange="toggleUSel(\''+u.id+'\',this)"></td><td><span class="mav" style="background:'+(p?.av||'#2563EB')+'">'+((p?.name||'?')[0])+'</span>'+(p?.name||'—')+'</td><td style="font-size:12px;color:var(--tx2);">'+(p?.dept||'—')+'</td><td><span class="tag '+(tc[u.role]||'gr')+'">'+u.rl+'</span></td><td style="font-size:12px;color:var(--tx3);">'+perm[u.role]+'</td></tr>'; }).join('');
}
...
function showBatchPerm(){
  var sel=Object.entries(userSel).filter(function (e) { return e[1]; }).map(function (e) { return e[0]; }); if(!sel.length){toast('请先勾选用户','err');return;}
  var m=document.createElement('div');m.className='mod';m.style.display='flex';m.onclick=function (e) { if(e.target===this)this.remove(); };
...
function batchRole(uids,r){
  ...
  uids.split(',').forEach(function (uid) { var u=USER_DATA.find(function (x) { return x.id===uid; }); if(u){u.role=r;u.rl=lb[r];} });
